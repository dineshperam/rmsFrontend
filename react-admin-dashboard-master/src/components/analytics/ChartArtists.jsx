...
const ChartArtists = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch(`http://localhost:8080/insights/top5artists?year=${selectedTimeRange}`);
				const data = await response.json();

				if (data.length > 0) {
					// Extract artist names dynamically from the first row (assuming all rows have same artist keys)
					const extractedKeys = Object.keys(data[0]).filter(key => key !== "month");
