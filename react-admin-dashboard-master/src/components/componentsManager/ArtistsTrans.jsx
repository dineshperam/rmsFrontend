...
const ArtistsTrans = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8080/trans/showTransByManId/${userId}`, {
  ...
  const handleSearch = (e) => {
    ...
    setFilteredTransactions(
      transactions.filter((transaction) => 
        new Date(transaction.transactionDate).toLocaleDateString().toLowerCase().includes(term)
      )
    );
  };
